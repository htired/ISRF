...
parser.add_argument('--L_user', type=int, default=4,
                    help='number of LightGCN layers')
# -----
...
all_iterator = TrainBatchify(exp_corpus.train, seq_corpus.user2items_positive, args.negative_num, nitem, tokenizer, args.exp_len, args.batch_size,args.data_dir)
...
model.init_graph_embeddings(args.alpha, args.sigma, args.L, args.L_user, exp_corpus.train, nuser, nitem, args.data_dir, args.k)
...
def train():
    ...
    align_sample = 0
    while True:
        task, source, source_mask, whole_word, target, users = all_iterator.next_batch()
        ...
        optimizer.zero_grad()
        outputs, a_loss = model(task, source, whole_word, source_mask, labels=target, users = users)
        ...
        if a_loss is not None:
            align_sample += batch_size
            align_loss += batch_size * a_loss.item()

        if all_iterator.batch_index % args.log_interval == 0 or all_iterator.batch_index % all_iterator.batch_num == 0:
            cur_t_loss = text_loss / total_sample
            cur_a_loss = 0.
            if align_sample != 0 :
                cur_a_loss = align_loss / align_sample
            print(now_time() + 'text loss {:4.4f}, align loss {:4.4f}  | {:5d}/{:5d} batches'.format(cur_t_loss,cur_a_loss, all_iterator.batch_index, all_iterator.batch_num))
            align_loss = 0.
            text_loss = 0.
            total_sample = 0
            align_sample = 0
...
def evaluate(iterator):
    ...
    with torch.no_grad():
        while True:
            task, source, source_mask, whole_word, target, users = iterator.next_batch_valid()
            task = task.to(device)  # (batch_size,)
            source = source.to(device)  # (batch_size, seq_len)
            source_mask = source_mask.to(device)
            whole_word = whole_word.to(device)
            ...
            if iterator.step == iterator.total_step:
                break
    if align_sample == 0:
        return text_loss / total_sample, 0
    return text_loss / total_sample, align_loss / align_sample
...
for epoch in range(1, args.epochs + 1):
    ...
    val_align_loss = (topn_a_loss + seq_a_loss + exp_a_loss) / 3

    print(now_time() + 'total loss {:4.4f}, total_align_loss{:4.4f}'.format(val_loss, val_align_loss))
    # Save the model if the validation loss is the best we've seen so far.
    if val_loss < best_val_loss:
        best_val_loss = val_loss
        with open(model_path, 'wb') as f:
